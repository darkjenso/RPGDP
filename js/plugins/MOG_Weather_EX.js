...
/*:
 * @plugindesc (v1.0) Adiciona novos efeitos de climas.
 * @author Moghunter
 *
 * @help  
 * =============================================================================
...
Spriteset_Base.prototype.initialize = function() {	
	...
	if ($gameSystem.weather_data[2] != null) {$gameSystem.weather_data[0] = true}
};

//==============================
// * createLowerLayer
//==============================
var _alias_mog_weather_ex_sprtbase_createLowerLayer = Spriteset_Base.prototype.createLowerLayer;
Spriteset_Base.prototype.createLowerLayer = function() {
     _alias_mog_weather_ex_sprtbase_createLowerLayer.call(this);
	 this._Weather_Plane = new Weather_EX_Plane();
	 this.addChild(this._Weather_Plane);
};
...
Weather_EX_Plane.prototype.refresh_weather_ex = function() {
	...
	this._weather_ex_sprites = [];    
	var w_type = $gameSystem.weather_data[1][0];	
	var w_power = Math.min(Math.max($gameSystem.weather_data[1][1] * 10,10),9999);
	var w_file_name = $gameSystem.weather_data[1][2];
...
Sprite_Weather_EX.prototype.need_refresh = function() {
	if (this.x < -(this._sc_range[0] + this._sc_range2[0]) || this.x > (this._sc_range[1] + this._sc_range2[1])) {return true};	
	if (this.y < -(this._sc_range[2] + this._sc_range2[2]) || this.y > (this._sc_range[3] + this._sc_range2[3])) {return true};
	return false;
};
...
Sprite_Weather_EX.prototype.random_zoom = function() {
	...
	this.scale = new PIXI.Point(0.5 + pz, 0.5 + pz);
};
...
Sprite_Weather_EX.prototype.setup_wind = function() {
	...
	this._n_sx = Math.min(Math.max((Math.random() * 5),2),8);
	this._n_sy = Math.min(Math.max((Math.random() * 5),2),8);	
	this.rotation = Math.random(360)
	this._n_rotation = Math.min(Math.max((Math.random() * 0.05),0.01),0.05);
	this.random_zoom();
	this.anchor.x = 1.0;
	this.anchor.y = 1.0;	
}
...
Sprite_Weather_EX.prototype.setup_snow = function() {
	...
	this._n_sx = 0;
	this._n_sy = Math.min(Math.max((Math.random() * 4),2),6);
	this.rotation = Math.random(360);
...
Sprite_Weather_EX.prototype.setup_rain = function() {
	...
	if (this._initial_position) { this.random_position(); this._initial_position = false; }
	   ...
	   this._n_x = -this._sc_range2[0] + this.screen_rx() + Math.floor((Math.random() * (Graphics.boxWidth + this._sc_range[0] + (this._sc_range2[0] * 2) )) - this._sc_range[0]);
       this._n_y = -(this._sc_range[2] + this._sc_range2[2]) + this.screen_ry()
	};
	this._n_sx = 0;
